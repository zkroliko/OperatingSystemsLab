#include "list.h"
	...
	void List_push_back(List* list, Data* data) {
		...
		if (element == NULL) {
			fprintf(stderr, "Bad allocation. Element haven't been added to the list!\n");
			return;					
		}
		element->data = Data_copy(data); // Copying data
		// Checking the allocation once again this time for copied data
	...
	Data* List_pop_front(List* list){
		// Checking if the list isn't empty
		if (list->first == NULL) {
			fprintf(stderr, "Trying to remove an element from an empty list!\n");
	...
	void List_sort(List* list) {
		...
		do {
			while (Data_compare(cur_element->data, consd_pos->prev->data) < 0 && (consd_pos != list->first)) {
